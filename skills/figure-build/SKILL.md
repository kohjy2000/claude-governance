...
If any required input missing → STOP, instruct user to run prerequisite (`/init-project`, `/figure-init`).
...
- Inputs to sub-skill:
  ...
  - Relevant `docs/` excerpts (CLAIMS entries for this figure's Group, STORY for Fig{N})
...
- Concurrency (parallel mode): use `Agent` tool with `subagent_type: figure-reviewer` for Layer 0-1 review, or `general-purpose` for parallel panel builds
- Idempotency: re-running with same inputs + state should be safe (resume picks up where left off)
- Multi-figure batch (e.g., `target=all` or `target=Fig1,Fig2,Fig3`): future enhancement, not v1.0

## Versioning
State file emits `_meta`:
```yaml
_meta:
  figure-build_version: "1.0.0"
