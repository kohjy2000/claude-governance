...
> 3. `figure-init` + `figure-build` (read claim groups to map Fig{N} → claims)
> 4. Users (reference when editing CLAIMS_template.md)
>
...
>
> **v1.1 changes**: Hierarchical claim ID (`C{group}-{N}`) replacing flat `C{N}`.
> Claim `Group` field added. Tag field (main/supp/discussion/deprecated) retained.
...
claim.id.pattern  : ^C[0-4]-\d+$
stale.main_claim_days   : 90
stale.any_claim_days    : 180
```

## Claim Group Semantics

`Group` field determines which main figure the claim maps to by default. `figure-init` uses this mapping when generating `Fig{N}_TARGET.md`.

| Group | Canonical meaning | Default figure |
|-------|-------------------|----------------|
...
Projects with fewer/more figures override via `figure-init figure_count=N`.
...
Each claim is a level-3 heading `### C{group}-{N}` where `{group}` ∈ `enum.Group` and `{N}` is a per-group integer (1, 2, 3, ...). Example: `### C1-3`. Pattern: `claim.id.pattern`.

Fields use `- **FieldName**: value` on a single line (except `Revision history`, which is nested bullets).

### Required fields

| Field | Type | Allowed values / format | Why required |
|-------|------|------------------------|--------------|
| Group | enum | `enum.Group` (C0..C4) | Default figure mapping (figure-init reads this) |
...
| Target figures | string | `Fig{N}{panel} (focal\|supporting\|mention)` or `none`. If absent, figure-init defaults by Group. | Figure placement |
...
### Group vs Tag distinction

- `Group` = which paper section / which figure (C0=method Fig1, C1=core Fig2, ...). Narrative structure.
- `Tag` = how prominent within its figure (main=focal, supp=ED, discussion=text only, deprecated=excluded). Placement prominence.

예: `Group: C2, Tag: supp` → Fig 3의 supplementary panel. `Group: C1, Tag: main` → Fig 2의 main panel.
...
2. All `### C{group}-{N}` IDs match `claim.id.pattern` (C0..C4 groups) and are unique across the file. IDs are never reused.
...
- v1.0 → v1.1 changes (2026-04-15): added required `Group` field, changed claim ID from `C{N}` to `C{group}-{N}`. Technically breaking for pre-existing files, but no pre-v1.2 project had real CLAIMS.md data, so no migration tooling shipped.
- Future versions add optional fields only. Removing a required field = v2.0 (breaking).
- No migration tooling in Phase 1-6. Future location: `~/.claude/blueprints/migrations/`.
...
- **`D{N}` namespace** — single `C{group}-{N}` namespace with Tag is simpler.
...
- **More than 5 groups (C5+)** — current project scope is 5-figure Nature-style paper. If >5 figures become common, extend `enum.Group` at that point.
